drSortBy<template>
  <div class="Driver">
      <!-- Driver slider -->
      <driver-size-slider style="margin-bottom:60px"
      ...
      <!-- Driver buttons -->
      <template>
        <a class="button is-small is-success" @click="allDrivers">All</a>
        ...
        <span> | </span>
        <a class="button is-small" v-bind:class="{ activeSortDriver:drSortBy == 'speedGround' }"@click="drSortSpeed">Speed</a>
        <a class="button is-small" v-bind:class="{ activeSortDriver:drSortBy == 'acceleration' }"@click="drSortAcceleration">Acceleration</a>
        <a class="button is-small" v-bind:class="{ activeSortDriver:drSortBy == 'weight' }"@click="drSortWeight">Weight</a>
        <a class="button is-small" v-bind:class="{ activeSortDriver:drSortBy == 'handlingGround' }"@click="drSortHandling">Handling</a>
        <a class="button is-small" v-bind:class="{ activeSortDriver:drSortBy == 'traction' }"@click="drSortTraction">Traction</a>
      </template>

      <hr>

      <template v-for="driverGroup in driverGroupList">
        <template v-if="driverGroup != null"> <!-- put a v-if before to force load before v-for -->
          <template v-for="driver in driverGroup">

            <a v-if="driver.size=='Light'" class="button driver-btn is-light-bg">
              <img :src="driver.image" :title="driver.name">
              <!-- <p class="driver-name">{{ driver.name }}</p> -->
            </a>

            <a v-if="driver.size=='Medium'" class="button driver-btn is-medium-bg">
              <img :src="driver.image" :title="driver.name">
              <!-- <p class="driver-name">{{ driver.name }}</p> -->
            </a>

            <a v-if="driver.size=='Heavy'" class="button driver-btn is-heavy-bg">
              <img :src="driver.image" :title="driver.name">
              <!-- <p class="driver-name">{{ driver.name }}</p> -->
            </a>
          ...
          <template v-if="drSortBy == 'speedGround'">
            <progress-bar
            :type="'success'"
            :size="'small'"
            :value="driverGroup[0].speedGround"
            :max="5"
            :show-label="true">
            </progress-bar>
          </template>

          <template v-if="drSortBy == 'acceleration'">
            <progress-bar
            :type="'success'"
            :size="'small'"
            :value="driverGroup[0].acceleration"
            :max="5"
            :show-label="true">
            </progress-bar>
          </template>

          <template v-if="drSortBy == 'weight'">
            <progress-bar
            :type="'success'"
            :size="'small'"
            :value="driverGroup[0].weight"
            :max="5"
            :show-label="true">
            </progress-bar>
          </template>

          <template v-if="drSortBy == 'handlingGround'">
            <progress-bar
            :type="'success'"
            :size="'small'"
            :value="driverGroup[0].handlingGround"
            :max="5"
            :show-label="true">
            </progress-bar>
          </template>

          <template v-if="drSortBy == 'traction'">
            <progress-bar
            :type="'success'"
            :size="'small'"
            :value="driverGroup[0].traction"
            :max="5"
            :show-label="true">
            </progress-bar>
          </template>
...
export default {
  ...
  components: {
    ...
    'driver-speed-slider' : vueSlider,
    'progress-bar': ProgressBar,
	},
	data () {
    return {
      drSortBy: 'speedGround',
      drSizeFilter: [1, 16],
      drSpeedFilter: [0, 5],
      drivers: [
  ...
  methods : {
    ...
    // },
    drSortSpeed() {
      this.drSortBy = 'speedGround'
    },
    drSortAcceleration() {
      this.drSortBy = 'acceleration'
    },
    drSortWeight() {
      this.drSortBy = 'weight'
    },
    drSortHandling() {
      this.drSortBy = 'handlingGround'
    },
    drSortTraction() {
      this.drSortBy = 'traction'
    },
  },
  computed : {
    testVal() {
    ...
    driverList() {
      return this.drivers.filter((driver) => {
        ...
        driver.speedGround >= this.drSpeedFilter[0] && driver.speedGround <= this.drSpeedFilter[1]
      })
    },
    // sortedDriverList() {
    //   switch (this.drSortBy) {
    ...
    driverGroupList() {
      ...
      switch (this.drSortBy) {
        ...
        case 'acceleration':
          ...
          for (var i = 0; i < this.driverList.length; i++) {
            if (arr.indexOf(this.driverList[i].acceleration) == -1) {
              arr.push("'" + this.driverList[i].acceleration + "'") // unordered keys fix
            }
          }
          arr.sort().reverse();
          for (var i = 0; i < arr.length; i++) {
            for (var j = 0; j < this.driverList.length; j++) {
              var k = "'" + this.driverList[j].acceleration + "'"; // as above
              if (k === arr[i]) {
                temp.push(this.driverList[j])
              }
            }
        ...
        case 'weight':
          ...
          for (var i = 0; i < this.driverList.length; i++) {
            if (arr.indexOf(this.driverList[i].weight) == -1) {
              arr.push("'" + this.driverList[i].weight + "'") // unordered keys fix
            }
          }
          arr.sort().reverse();
          for (var i = 0; i < arr.length; i++) {
            for (var j = 0; j < this.driverList.length; j++) {
              var k = "'" + this.driverList[j].weight + "'"; // as above
              if (k === arr[i]) {
                temp.push(this.driverList[j])
              }
            }
        ...
        case 'handlingGround':
          ...
          for (var i = 0; i < this.driverList.length; i++) {
            if (arr.indexOf(this.driverList[i].handlingGround) == -1) {
              arr.push("'" + this.driverList[i].handlingGround + "'") // unordered keys fix
            }
          }
          arr.sort().reverse();
          for (var i = 0; i < arr.length; i++) {
            for (var j = 0; j < this.driverList.length; j++) {
              var k = "'" + this.driverList[j].handlingGround + "'"; // as above
              if (k === arr[i]) {
                temp.push(this.driverList[j])
              }
            }
        ...
        case 'traction':
          ...
          for (var i = 0; i < this.driverList.length; i++) {
            if (arr.indexOf(this.driverList[i].traction) == -1) {
              arr.push("'" + this.driverList[i].traction + "'") // unordered keys fix
            }
          }
          arr.sort().reverse();
          for (var i = 0; i < arr.length; i++) {
            for (var j = 0; j < this.driverList.length; j++) {
              var k = "'" + this.driverList[j].traction + "'"; // as above
              if (k === arr[i]) {
                temp.push(this.driverList[j])
              }
            }
